...
const removeBlock = async (req, res, err) => {
    ...
    if (bot) {
        await Block.findOneAndDelete({ $and: [{ _id: req.params.blockid }, { bot: req.params.botid }] });
...
const editBlock = async (req, res, err) => {
    ...
    if (bot) {
        const blockData = req.body.data;
        const block = await Block.findOneAndUpdate({ $and: [{ bot: req.params.botid }, { _id: req.params.blockid }] }, { ...blockData }, { new: true });
        return res.json({ block });
    }
    res.sendStatus(404);
};
const getBlocks = async (req, res, err) => {
    const bot = await Bot.findOne({ $and: [{ user_id: req.user._id }, { _id: req.params.botid }] });
    if (bot) {
        const blocks = await Block.find({ bot: req.params.botid });
...
const getBlock = async (req, res, err) => {
    const bot = await Bot.findOne({ $and: [{ user_id: req.user._id }, { _id: req.params.botid }] });
    if (bot) {
        const block = await Block.findOne({ $and: [{ bot: req.params.botid }, { _id: req.params.blockid }] });
        if (block) {
            const templates = await Block_template.find({ block: block._id });
            return res.json({ block, templates });
        }
    }
    res.sendStatus(404);
};
...
const createBlockTemplate = async (req, res, err) => {
    ...
    if (bot) {
        ...
        try {
            ...
            const block = await Block.findOne({ $and: [{ bot: bot._id }, { _id: req.params.blockid }] });
            if (!!block) {
...
const removeBlockTemplate = async (req, res, err) => {
    ...
    if (bot) {
        const block = await Block.findOne({ $and: [{ bot: bot._id }, { _id: req.params.blockid }] });
        if (!!block) {
            const block_template = await Block_template.findOneAndRemove({ $and: [{ _id: req.params.templateid }, { block: block._id }] });
            if (block_template)
                return res.sendStatus(200);
        }
    }
...
const editBlockTemplate = async (req, res, err) => {
    ...
    if (bot) {
        const block = await Block.findOne({ $and: [{ bot: bot._id }, { _id: req.params.blockid }] });
        if (!!block) {
            const template = await Block_template.findOneAndUpdate({ $and: [{ _id: req.params.templateid }, { block: block._id }] }, { ...templateDate }, { new: true });
            return res.json({ template });
        }
    }
    res.sendStatus(404);
